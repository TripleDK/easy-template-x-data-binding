...
var easyTemplateX = require('easy-template-x');
...
class CustomXmlFiles {
  ...
  async load() {
    ...
    for (const path of this.zip.listFiles()) {
      ...
      const filename = easyTemplateX.Path.getFilename(path);
...
class DataBindingExtension extends easyTemplateX.TemplateExtension {
  /**
  ...
  constructor(plugins) {
    ...
    this.pluginsLookup = easyTemplateX.toDictionary(plugins, p => p.contentType);
  }
  ...
  findNodes(node) {
    const nodes = [];
    const depth = new easyTemplateX.XmlDepthTracker(this.maxXmlDepth);
  ...
  findNextNode(node, depth) {
    ...
    if (node.childNodes && node.childNodes.length) {
      depth.increment();
      return easyTemplateX.first(node.childNodes);
    } // siblings
  ...
  isMatch(node) {
    if (node.nodeType === easyTemplateX.XmlNodeType.Text) {
      return false;
    }
    ...
    if (easyTemplateX.first(node.childNodes).nodeType === easyTemplateX.XmlNodeType.Text) {
      return true;
    }
  ...
  updateNode(node, data) {
    ...
    if (!plugin) {
      throw new easyTemplateX.UnknownContentTypeError(contentType, value, data.path.join("."));
    }
...
class DataBindingTemplatePlugin extends easyTemplateX.TemplatePlugin {
  constructor(...args) {
    super(...args);
  ...
  setNodeContents(node, content) {
    const contentNode = easyTemplateX.XmlNode.createTextNode(this.convertToDataBindingValue(content.value));
    easyTemplateX.XmlNode.remove(easyTemplateX.XmlNode.lastTextChild(node));
    easyTemplateX.XmlNode.appendChild(node, contentNode);
  }
  /**
...
class DataBindingDatePlugin extends DataBindingTemplatePlugin {
  ...
  convertToDataBindingValue(value) {
    ...
    return easyTemplateX.first(date.toISOString().split("T"));
  }
...
exports.CustomXmlFiles = CustomXmlFiles;
exports.DataBindingBooleanPlugin = DataBindingBooleanPlugin;
exports.DataBindingDatePlugin = DataBindingDatePlugin;
exports.DataBindingExtension = DataBindingExtension;
exports.DataBindingPluginContent = DataBindingPluginContent;
exports.DataBindingTemplatePlugin = DataBindingTemplatePlugin;
exports.DataBindingTextPlugin = DataBindingTextPlugin;
exports.XmlNodePath = XmlNodePath;
exports.createDefaultDataBindingPlugins = createDefaultDataBindingPlugins;
